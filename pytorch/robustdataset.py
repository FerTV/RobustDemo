...
class RobustDataset(Dataset, ABC):
    """
    ...
    def plot_data_distribution(self, dataset, partitions_map):
        ...
        for i in range(self.number_sub):
            ...
            plt.tight_layout()
            path_to_save = f"{self.config.participant['tracking_args']['log_dir']}/participant_{i+1}_class_distribution_{'iid' if self.iid else 'non_iid'}{'_' + self.partition if not self.iid else ''}.png"
            plt.savefig(
